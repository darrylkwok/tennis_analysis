...
class MiniCourt():
    ...
    def get_width_point_of_mini_court(self):
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            for player_id, bbox in player_bbox.items():
                ...
                # Get the closest key in pixels
                closest_key_point_index = get_closest_keypoint_index(foot_position, original_court_key_points, [0,2,12,13]) #we manually examine the court and took 0,2(baseline) and 12,13(middle of the court)
                ...
                frame_index_min = max(0, frame_num-20) # Get the smallest index - we try to get the player's height within a certain range of frames
                frame_index_max = min(len(player_boxes), frame_num+50) # Get the largest index
                ...
                if closest_player_id_to_ball == player_id:
                    # Get the closest key in pixels
                    closest_key_point_index = get_closest_keypoint_index(ball_position, original_court_key_points, [0,2,12,13]) #we manually examine the court and took 0,2(baseline) and 12,13(middle of the court)
                    closest_key_point = (original_court_key_points[closest_key_point_index*2],
                                     original_court_key_points[closest_key_point_index*2+1])
